...
def _get_req_param(body):
    group_id = get_model_configuration(VENDOR_MINIMAX, "group_id", body.model)
    jwt = get_model_configuration(VENDOR_MINIMAX, "jwt", body.model)
    api_base = get_base_url(VENDOR_MINIMAX, body.model)
    headers = {
        "Authorization": jwt,
